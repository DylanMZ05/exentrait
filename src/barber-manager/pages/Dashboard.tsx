...
export const Dashboard: React.FC = () => {
  // Estado para el contador real de empleados
  const [totalEmpleados, setTotalEmpleados] = useState<number | null>(null);

  /* =========================================================
     LOGIC: CACHE-FIRST + AUTH LISTENER
  ========================================================= */
  useEffect(() => {
    // Usamos onAuthStateChanged para asegurarnos de tener el usuario
    // ANTES de intentar leer el caché o Firebase.
    const unsubscribe = onAuthStateChanged(barberAuth, (user) => {
      if (!user) {
        setTotalEmpleados(null);
        return;
      }

      // Ahora que estamos 100% seguros de quién es el usuario:
      const cacheKey = `barber_stats_empleados_${user.uid}`;
      const cachedData = localStorage.getItem(cacheKey);

      if (cachedData) {
        // 1. SI ESTÁ EN CACHÉ: Usamos el dato local (Costo: $0)
        console.log("⚡ [Dashboard] Usando empleados desde caché local");
        setTotalEmpleados(Number(cachedData));
      } else {
        // 2. SI NO ESTÁ: Leemos de Firebase (Costo: 1 lectura)
        console.log("🔥 [Dashboard] Leyendo empleados desde Firebase (Cache miss)");
        const fetchFromFirebase = async () => {
          try {
            const empleadosRef = collection(
              barberDb,
              `barber_users/${user.uid}/empleados`
            );
