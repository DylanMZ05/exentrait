...
// Definimos los métodos de pago (ajustados a tu solicitud)
...
interface Venta {
    ...
    metodoPago?: PaymentMethod; // ⭐ NUEVO: Método de pago
}
interface Producto {
    nombre: string;
...
export const Dashboard: React.FC = () => {
    ...
    // ⭐ MODIFICAR EL CONFIG PARA INCLUIR ESTADOS DEL FINALIZAR TURNO
    const [confirmConfig, setConfirmConfig] = useState<{
        title: string;
        message: string;
        action: (metodoPago?: PaymentMethod) => Promise<void>; // La acción ahora puede recibir el método de pago
        confirmText: string;
        isDanger?: boolean;
        showPaymentMethod?: boolean; // ⭐ NUEVO: Muestra el selector de método de pago
    ...
    // ⭐ NUEVO ESTADO PARA CAPTURAR EL MÉTODO DE PAGO EN EL MODAL DE CONFIRMACIÓN RÁPIDA
    ...
    const [formMetodoPago, setFormMetodoPago] = useState<PaymentMethod>('Efectivo'); // ⭐ NUEVO ESTADO DE PAGO
    ...
    // ⭐ LÓGICA DE ORDENAMIENTO PARA EL POPUP DE VENTA
    const sortedBarbers = useMemo(() => {
        const activeBarbers = empleadosList.filter(e => e.activo !== false); // Filtra los activos
            ...
            salesSnap.forEach(doc => {
                ...
                if (loggedEmployee && data.barberId === loggedEmployee.id) {
                     if (data.tipo === 'Ingreso') {
                         // Sumamos solo SU PARTE (Venta * Porcentaje / 100). 
    ...
    const resetForm = useCallback(() => {
        ...
        if (hasServices && defaultService) {
            ...
            // Esto solo ocurriría si servicios.length fuera 0 pero el find encontrara algo (imposible, pero por si acaso)
             setFormDescripcion(`Venta de Servicio: ${defaultService.nombre}`);
             setFormMonto(defaultService.precio.toString());
             setFormTipo('Ingreso');
        }
        
        // ⭐ Usar sortedBarbers para preseleccionar y obtener el Barbero
        const firstBarber = sortedBarbers[0];
        const me = sortedBarbers.find(e => e.id === userUid);

        if (isEmployeeMode && me) {
             setFormBarberId(me.id); // Si soy empleado, me preselecciono
        } else if (firstBarber) {
             setFormBarberId(firstBarber.id); // Si soy dueño o no soy empleado, selecciono el primero de la lista ordenada
        } else {
             setFormBarberId('');
        }
    ...
    const handleSave = async () => {
        ...
        try {
             ...
             const data = {
                  ...
                  metodoPago: formTipo === 'Ingreso' ? formMetodoPago : 'N/A', // ⭐ NUEVO
    ...
    const executeConfirmAction = async () => {
        ...
        try {
            // ⭐ Pasar el método de pago si está activado
            if (confirmConfig.showPaymentMethod) {
                await confirmConfig.action(quickPayMethod);
            } else {
                await confirmConfig.action(); // Ejecuta la acción (Cancelar)
            }
            ...
            setQuickPayMethod('Efectivo'); // Resetear
        }
    };
    ...
    const triggerConfirm = (
        ...
        showPaymentMethod: boolean = false
          ...
          setQuickPayMethod('Efectivo'); // Siempre resetear al default
    ...
    const handleQuickFinalize = (turno: Turno) => {
          ...
          const action = async (metodoPago: PaymentMethod = 'Efectivo') => { // Recibe el método de pago
              const effectiveUid = localStorage.getItem('barberOwnerId') || userUid;
              ...
              const ventaRef = await addDoc(collection(barberDb, `barber_users/${effectiveUid}/ventas`), {
                  ...
                  metodoPago: metodoPago, // ⭐ APLICADO AQUÍ
                  createdAt: serverTimestamp(), 
              });
          ...
          triggerConfirm(
              ...
              false, 
              action,
              true // ⭐ Muestra el selector de método de pago
          );
    };
                 ...
                 <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
                      ...
                      >
                           ...
                           <div className="space-y-4">
                                ...
                                <div>
                                     ...
                                     <div className="flex space-x-4">
                                          <button 
                                               onClick={() => {
                                                    setFormTipo('Ingreso');
                                                    // ⭐ CAMBIO: Preseleccionar el barbero primero
                                                    const firstBarber = sortedBarbers[0];
                                                    if (firstBarber) setFormBarberId(firstBarber.id);

                                                    // ⭐ CAMBIO: Establecer VentaType a 'servicio' si hay servicios.
                                ...
                                {formTipo === 'Ingreso' && sortedBarbers.length > 0 && ( // ⭐ Usar sortedBarbers
                                                <div className="border-t border-slate-100 pt-4">
                                                     ...
                                                     >
                                                          ...
                                                          {sortedBarbers.map((e) => ( // ⭐ Usar sortedBarbers
            ...
            {confirmOpen && (
                 ...
                 >
                      ...
                      >
                           ...
                           {/* ⭐ Selector de Método de Pago para el Quick Finalize */}
